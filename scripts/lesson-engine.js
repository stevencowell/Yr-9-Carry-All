(function () {
  ...
  const STORAGE_KEY = config.storageKey || 'carry-all-guided-lesson';
  ...
  function downloadPdf() {
    ...
    const fileName = `${(state.studentName || 'student').replace(/[^a-z0-9]+/gi, '-').replace(/^-|-$/g, '')}-${(config.title || 'carry-all').replace(/[^a-z0-9]+/gi, '-').replace(/^-|-$/g, '')}.pdf`;
